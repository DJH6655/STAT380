...
for (i in 1:num_folds) {
  ...
  for (j in 2:maxK) {
    #Build Model
    rf_model <- randomForest(factor(loan_status) ~ no_of_dependents +
                     education + self_employed +
                     income_annum + loan_amount +
                     loan_term + cibil_score +
                     residential_assets_value +
                     commercial_assets_value +
                     luxury_assets_value +
                     bank_asset_value , data = Train, ntree = j, mtry = 3)
    
    pred <- predict(rf_model, newdata = Test[ , xvars])
    
    #Add Predictions to test set
    Test <- Test %>%
      mutate(pred_status = pred)
...
From this plot, accuracy increases by less around ntree = 13, so this seems like a good number to use. Let's use ten fold cross validation and get model metrics:
...
for (i in 1:num_folds) {
  ...
  #Separate predictions and results
  pred_rf <- factor(Test$pred_status)
  act_rf <- factor(Test$loan_status)
  
  # Calculate Accuracy and F1 score
  accuracy <- mean(pred_rf == act_rf)
  
  metrics <- confusionMatrix(pred_rf,
                             act_rf,
                             mode = "everything", positive = "1") 
...
The accuracy and F1 scores are even higher here, suggesting that the random forest model outperforms Logistic Regression and kNN.
